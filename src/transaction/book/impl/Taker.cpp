...
BasicTaker::Rate::divide (Amount const& amount) const
{
    ...
    return skywell::divide (amount, rate_, amount.issue ());
}
...
BasicTaker::Rate::multiply (Amount const& amount) const
{
    ...
    return skywell::multiply (amount, rate_, amount.issue ());
}
...
{
    return isSWT (issuer) || (account == issuer) ? QUALITY_ONE : skywellTransferRate (view, issuer);
}
