...
// - A skywell nodes output issuer must be the node's account or the next node's
//   account.
    ...
    Account const& issuer)    // --> Delivering this issuer.
{
    ...
    // For skywell, non-SWT, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to skywell through issuer's account.
    ...
    Account const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a skywell account. false, iff node is an offer node.
...
/** Check if a sequence of three accounts violates the no skywell constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no skywell on [first]->[second] and
    [second]->[third]
*/
...
{
    // fetch the skywell lines into and out of this node
...
{
    // There must be at least one node for there to be two consecutive skywell
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    // These are the nodes whose no skywell constraints could be violated
