...
TER PathCursor::forwardLiquidityForAccount () const
{
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to skywell based on what is
            // available.
        ...
        else if (nodeIndex_ == lastNodeIndex)
        {
            ...
            {
                // Actually receive.
                resultCode = ledger().skywellCredit (
                    previousAccountID,
                    node().account_,
        ...
        else
        {
                ...
                // Current wants to issue.
            {
                ...
                skywellLiquidity (
                    skywellCalc_,
                    QUALITY_ONE,
                    skywellTransferRate(ledger(), node().account_, node().saFwdIssue.getCurrency()),
                    previousNode().saFwdRedeem,
                    node().saRevIssue,
                    saPrvRedeemAct,
                    node().saFwdIssue,
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
                ...
                // Previous wants to redeem.
            {
                ...
                skywellLiquidity (
                    skywellCalc_,
                    QUALITY_ONE,
                    skywellTransferRate(ledger(), node().account_, node().saFwdDeliver.getCurrency()),
                    previousNode().saFwdRedeem,
                    node().saRevDeliver,
                    saPrvRedeemAct,
                    node().saFwdDeliver,
        ...
        else
        {
            ...
            else if (!isSWT (node().issue_))
            {
                ...
                // which skywell balance will be adjusted.  Assume just issuing.
    ...
    else
    {
        ...
        // if (skywell::legacy::emulate027 (skywellCalc_.mActiveLedger.getLedger ()))
