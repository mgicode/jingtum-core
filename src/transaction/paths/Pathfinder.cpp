...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.skywell.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
...
bool Pathfinder::isNoSkywell (Account const& fromAccount
                            , Account const& toAccount
                            , Currency const& currency)
{
    auto sleSkywell = mLedger->getSLEi (getSkywellStateIndex (toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount) ? lsfHighNoSkywell : lsfLowNoSkywell);
...
bool Pathfinder::isNoSkywellOut (STPath const& currentPath)
{
    ...
    // no skywell on the output. A path with no skywell on its output can't be
    // followed by a link with no skywell on its input.
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                auto& skywellLines (mRLCache->getSkywellLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (skywellLines.size ());

                for (auto const& item : skywellLines)
                {
