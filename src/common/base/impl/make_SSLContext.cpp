...
make_DH(std::string const& params)
{
    ...
    if (p == nullptr)
        skywell::FatalError ("d2i_DHparams returned nullptr.",
            __FILE__, __LINE__);
    return dh_ptr(dh);
}
...
getDH (int keyLength)
{
    ...
    {
        skywell::FatalError ("unsupported key length", __FILE__, __LINE__);
    }
...
disallowRenegotiation (SSL const* ssl, bool isNew)
{
    ...
    static skywell::static_initializer <StaticData> static_data;
    ...
    std::string key_file, std::string cert_file, std::string chain_file)
{
    ...
    if (! cert_file.empty ())
    {
        ...
        if (ec)
        {
            skywell::FatalError (error_message (
    ...
    if (! chain_file.empty ())
    {
            ...
            for (;;)
            {
                ...
                if (! cert_set)
                {
                    if (SSL_CTX_use_certificate (ssl, x) != 1)
                        skywell::FatalError ("Problem retrieving SSL certificate from chain file.",
                            __FILE__, __LINE__);
                ...
                {
                    X509_free (x);
                    skywell::FatalError ("Problem adding SSL chain certificate.",
                        __FILE__, __LINE__);
                }
            }
        ...
        {
            fclose (f);
            skywell::FatalError ("Reading the SSL chain file generated an exception.",
                __FILE__, __LINE__);
        }
    }
    ...
    {
        skywell::FatalError ("Invalid key in SSL private key file.",
            __FILE__, __LINE__);
    }
}
