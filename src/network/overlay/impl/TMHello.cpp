...
appendHello (beast::http::message& m, protocol::TMHello const& hello)
{
    ...
    h.append ("Session-Signature", skywell::base64_encode (hello.nodeproof()));
    ...
    if (hello.has_ledgerclosed())
        h.append ("Closed-Ledger", skywell::base64_encode (hello.ledgerclosed()));

    if (hello.has_ledgerprevious())
        h.append ("Previous-Ledger", skywell::base64_encode (hello.ledgerprevious()));
}

std::vector<ProtocolVersion>
parse_ProtocolVersions (std::string const& s)
{
    static skywell::static_initializer<boost::regex> re (
        "^"                  // start of line
...
parseHello (beast::http::message const& m, beast::Journal journal)
{
...

    {
        ...
        hello.set_nodeproof (skywell::base64_decode (iter->second));
    }
...

    {
        ...
        if (iter != h.end())
            hello.set_ledgerclosed (skywell::base64_decode (iter->second));
    }

    {
        auto const iter = h.find ("Previous-Ledger");
        if (iter != h.end())
            hello.set_ledgerprevious (skywell::base64_decode (iter->second));
    }
