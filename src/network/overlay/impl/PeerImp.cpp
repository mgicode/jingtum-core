...
PeerImp::run()
{
    ...
    {
        auto s = getVersion();
        if (boost::starts_with(s, "skywelld-"))
        {
            s.erase(s.begin(), s.begin() + 8);

            skywell::SemanticVersion v;
            if (v.parse(s))
            {
                skywell::SemanticVersion av;
                av.parse("0.28.1-b7");
                hopsAware_ = v >= av;
            }
        }
...
PeerImp::onMessageBegin (std::uint16_t type, std::shared_ptr<::google::protobuf::Message> const& m)
{
    ...
    fee_ = skywell::Resource::feeLightPeer;
...
PeerImp::onMessage (std::shared_ptr <protocol::TMLedgerData> const& m)
{
    ...
    if (m->ledgerhash ().size () != 32)
    {
        ...
        fee_ = skywell::Resource::feeInvalidRequest;
...
PeerImp::onMessage (std::shared_ptr<protocol::TMProposeSet> const& m)
{
    ...
    SkywellAddress signerPublic = SkywellAddress::createNodePublic (strCopy (set.nodepubkey ()));
