...
OverlayImpl::onPrepare()
{
    ...
    if (bootstrapIps.empty ())
        bootstrapIps.push_back ("r.skywell.com 51235");
    ...
    // Add the ips_fixed from the skywelld.cfg file
...
void
OverlayImpl::onPeerDeactivate (Peer::id_t id, SkywellAddress const& publicKey)
{
    std::lock_guard <decltype(mutex_)> lock (mutex_);
...
OverlayImpl::crawl()
{
    ...
    for (auto const& e : m_publicKeyMap)
    {
        if (auto const sp = e.second.lock ())
        {
            ...
            pv[jss::type] = "peer";
            pv[jss::public_key] = skywell::base64_encode(sp->getNodePublic().getNodePublic().data()
