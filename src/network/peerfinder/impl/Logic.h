    ...
    */
                        ...
                        boost::system::error_code ec)
    {
        ...
        if (ec)
        {
            ...
            if (m_journal.error) 
                m_journal.error << skywell::leftw (18) 
                                << "Logic testing " 
                                   ...
                                   boost::asio::ip::tcp::endpoint const& remote_endpoint)
    {
        ...
        /*
        {
            ...
            if (iter != state->slots.end ())
            {
                ...
                //assert (consistent);
                if (m_journal.warning) m_journal.warning << skywell::leftw (18) <<
        ...
        // Check for self-connect by address
        {
            ...
            if (iter != state->slots.end ())
            {
                ...
                if (m_journal.warning) 
                    m_journal.warning << skywell::leftw (18) 
                                      << "Logic dropping "
                                      << convert_endpoint_to_string(slot->remote_endpoint ())
                                      << " as self connect";
    ...
    Result
    activate (SlotImp::ptr const& slot, SkywellPublicKey const& key, bool cluster)
    {
        if (m_journal.debug)
            m_journal.debug << skywell::leftw (18) 
                            << "Logic handshake " 
    ...
    */
    ...
    void on_endpoints (SlotImp::ptr const& slot, Endpoints list)
    {
        ...
        for (auto const& ep : list)
        {
            ...
            if (ep.hops == 1)
            {
                ...
                {
                    if (m_journal.debug) m_journal.debug << skywell::leftw (18) 
                                                         << "Logic testing " 
                                                         << convert_endpoint_to_string(ep.address)
                                                         << " already in progress";
                    continue;
    ...
    void fetch (std::shared_ptr<Source> const& source)
    {
        ...
        if (! results.error)
        {
            ...
            if (m_journal.info) 
                m_journal.info << skywell::leftw (18) 
                               << "Logic added " 
                               << count 
                               << " new "
